    ...
    function SaveConfigService($q, $log, BaseService, BaseConfigService, RemoteService, PageErrorDataService, LocationDataService, PricingMatrixDataService, OptionGroupDataService, ProductAttributeValueDataService, ProductAttributeConfigDataService, PAVObjConfigService, ConstraintRuleDataService) {
                                ...
                                && isProdSelected(productIdtoComponentMap[parentId])))
                        {
                            _.each(optiongroup.productOptionComponents, function(productcomponent){
                                ...
                                // {
                                    ...
                                    productcomponent = _.omit(productcomponent, ['$$hashKey', 'isDisabled', 'isAvailableonSLocation', 'product', 'sensitiveHelpFieldsMap']);
            ...
            requestPromise.then(function(result){
                ...
                */
                ...
                _.each(constraintActionDoList, function(ActionDo){
                    // get all error messages and add to PageErrorDataService.
                        ...
                        // || ActionType == 'Replacement')
                    {
                        switch(ActionIntent)
                        {
                            ...
                            case 'Show Message':
                                if(ActionType == 'Validation'){
                                    PageErrorDataService.add(Message);
                                }
        ...
        function runClientValidations(){
                ...
                && hasLocations)
            {
                ...
                PageErrorDataService.add('Please select location to Proceed.');
                res = false;
            }
            ...
            _.each(allOptionGroups, function(optiongroups, bundleprodId){
                _.each(optiongroups, function(optiongroup){
                            ...
                            && isProdSelected(productIdtoComponentMap[parentId])))
                    {
                        ...
                        {
                            PageErrorDataService.add('Minimum of '+minOptions+' options have to be selected in '+optiongroup.groupName);
                            res = false;
                        }
                        if(maxOptions > 0
                            && selectedOptionsCount > maxOptions)
                        {
                            PageErrorDataService.add('Maximum of '+maxOptions+' options can to be selected from '+optiongroup.groupName);
                            res = false;
                        }
                    }
                })
